 ...
 * <p/>
 * The input is configured by "repository.inputdirs", which can be a comma
 * seperated list of folders, or an array, e.g. multiple
 * "+repository.inputdirs=...". The dirs are scanned recursively for input
 ...
 * "repository.defaultentityreader". Alternatively, different entityreaders can
...
public class EntityReaderInputFormat extends FileInputFormat<LongWritable, EntityWritable> {
   ...
   public EntityReaderInputFormat(Job job) {
      ...
      String inputdirs[] = conf.get("repository.inputdir").split(",");
   ...
   public void loadEntityReaderSettings(Configuration conf) {
      defaultentityreader = conf.getSubString("repository.defaultentityreader", EntityReaderTrec.class.getCanonicalName());
