...
public abstract class GraphComponent {
   ...
   public void remove(Operator remove) {
      for (int f = 0; f < containednodes.size(); f++) {
         ...
         if (feature == remove) {
            ...
            if (!(feature instanceof QTerm) && feature.containednodes.isEmpty()) {
               containednodes.remove(f--);
            }
         }
      }
   ...
   public ArrayList<QTerm> getStopWords() {
      ArrayList<QTerm> results = new ArrayList<QTerm>();
   ...
   public ArrayList<QTerm> getNonStopWords() {
      ArrayList<QTerm> results = new ArrayList<QTerm>();
      for (Operator g : containednodes) {
         if (g instanceof QTerm && !((QTerm)g).isStopword()) {
            results.add((QTerm)g);
