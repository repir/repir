 ...
 */
public abstract class Operator extends GraphComponent implements Comparator<Operator>, Comparable<Operator> {
   ...
   public final OperatorValues featurevalues;
   public int span = 1;
   ...
   protected Operator(GraphRoot root) {
      ...
      this.root = root;
      featurevalues = new OperatorValues();
   }
      ...
      for (Operator f : list) {
            for (Operator ff : list) {
               if (ff == f)
                  break;
               if (f.equals(ff)) {
                  f.setweight(ff.getweight() + f.getweight());
                  remove.add(ff);
                  break;
               }
            }
   ...
   public boolean needsCollect() {
      return (willbescored)
              && (featurevalues.cf < 0 || featurevalues.df < 0);
   }
   ...
   public OperatorValues getFeatureValues() {
   ...
   public double getweight() {
      return featurevalues.queryweight;
   }
   ...
   public void setcf(Long termfrequency) {
      featurevalues.cf = termfrequency;
   }

   public void setdf(Long documentfrequency) {
      featurevalues.df = documentfrequency;
   }
